...
function Popbox(b){
  ...
  this.start = function() {
      ...
      if (popinner) {
          ...
          popinner.style.top = window.document.documentElement.clientHeight / 2 - popinner.clientHeight / 2 + 'px'
      }
  }
  this.end = function() {
      var b = this;
  ...
  this.mini = function() {
      ...
      if(c){
        if(c.classList.toString().indexOf('popOut') > -1){
          c.classList.add('popIn')
        }
        c.style.left = (document.body.offsetWidth- c.clientWidth) / 2 + 'px'
        c.style.top = (window.document.documentElement.clientHeight - c.clientHeight) / 2 + 'px'
        setTimeout(function(){
...
function Toast(){
  ...
  this.start = function(txt){
    ...
    if(progress){
      ...
      progress.style.top = window.document.documentElement.clientHeight / 2 - progress.clientHeight / 2 + 'px'
    }
    this.HTMLRemove();
    new Popbox({type: "fullscreen"});
  }
